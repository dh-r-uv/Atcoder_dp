...
void solve()
{
	...
	cin >> n >> w;
	vii v(n+1);
	fr(1, n+1) cin >> v[i].F >> v[i].S;
	//vvi dp(n+1, vi(w+1));

	// fr(1, n+1){
	// 	rep(j, 0, w+1){
	// 		dp[i][j] = dp[i-1][j];
	// 		if (j >= v[i].F) dp[i][j] = max(dp[i][j], dp[i-1][j-v[i].F] + v[i].S);
	// 	}
	// }
	//cout << dp[n][w]<<endl;

	vvi dp(n+1, vi(100003, inf));
	//find the min weights to achieve that value
	dp[0][0] = 0;
	int ans = 0;
	fr(1, n+1){
		rep(j, 0, 100003){
			dp[i][j] = dp[i-1][j];
			if (j >= v[i].S) dp[i][j] = min(dp[i][j], dp[i-1][j-v[i].S] + v[i].F);
