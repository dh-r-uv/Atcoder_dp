...
Mint ways(int x1, int y1, int x2, int y2){
...
void solve()
{
    ...
    cin >> h >> w >> n;
    vii a(n);
    fr(0, n) {cin >> a[i].F >> a[i].S; a[i].F--; a[i].S--;}
    sort(all(a));
    a.pb({h-1, w-1});
    vector<Mint> dp(n+1);
    rep(i, 0, n+1){
        dp[i] = ways(0, 0, a[i].F, a[i].S); //ways to reach blocked cell
        rep(j, 0, i) dp[i]-=dp[j]*ways(a[j].F, a[j].S, a[i].F, a[i].S); //ways to blocked cell from previous blocked cells
    }
