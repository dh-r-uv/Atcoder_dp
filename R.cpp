...
//define the global variables here
const int mxN = 53;
vector<vector<Mint>> a(mxN, vector<Mint>(mxN, 0));


void matrix_mul(vector<vector<Mint>> &a, vector<vector<Mint>> &b, ll n){
    vector<vector<Mint>> res(mxN, vector<Mint>(mxN, 0));
    fr(0, n) rep(j, 0, n) rep(k, 0, n) res[i][j] += a[i][k]*b[k][j];
    a = res;
}

void matrix_bin_pow(vector<vector<Mint>> &a, ll n, ll k, vector<vector<Mint>> &r){
    fr(0, n) r[i][i] = 1;
    while(k){
        if(k&1) matrix_mul(r, a, n);
        matrix_mul(a, a, n);
        k >>= 1;
    }
    return;
...
void solve()
{
    ll n, k;
    cin >> n >> k;
    fr(0, n) rep(j, 0, n) cin >> a[i][j];
    //a^k[i][j] will indicate number of paths from i to j of length k
    vector<vector<Mint>> r(n, vector<Mint>(n, 0));
    matrix_bin_pow(a, n, k, r);
    Mint ans = 0;
    rep(i, 0, n) rep(j, 0, n) ans+=r[i][j];
    cout << ans <<endl;
