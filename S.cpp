...
vector<vector<Mint>> dp2(mxN, vector<Mint>(mxD));   //dp[i][j] -> no. of ways to get rem j for first i digits
...
void solve()
{
    ...
    int sm = 0;
    rep(i, 1, n+1){
        //for numbers less than sm
        rep(j, 0, d){   
            rep(k, 0, 10) dp2[i][(j+k)%d] += dp2[i-1][j];
        }
        //equal to sm
        int dig = s[i]-'0'; 
        rep(j, 0, dig) dp2[i][(sm+j)%d]++;
        sm+=dig;
    }
    if(sm%d!=0) dp2[n][0]--;
    cout << dp2[n][0] << endl;
